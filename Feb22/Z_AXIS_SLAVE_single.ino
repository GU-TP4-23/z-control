...
void receiveEvent(int howMany) // howMANY is always equal to no. of bytes received
  {                             //an int is 4 bytes, therefore taking up 4 spaces in the array                                    
    ...
    while(stepper.currentPosition() != z1)    // Run the motor forward at 800 steps/second until the motor reaches coord from array (1/16 revolutions)
      {
        stepper.setSpeed(800);         // sets speed in forward direction of stepper 
